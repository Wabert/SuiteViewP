...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('suiteview-ai-bridge.startServer', startServer),
        vscode.commands.registerCommand('suiteview-ai-bridge.stopServer', stopServer),
        vscode.commands.registerCommand('suiteview-ai-bridge.showStatus', showStatus)
    );
...
async function startServer() {
    ...
    server.listen(port, '127.0.0.1', () => {
        ...
        updateStatusBar(true, port as number);
    });
...
function showStatus() {
    ...
    if (server) {
        vscode.window.showInformationMessage(
            `SuiteView AI Bridge is running on http://127.0.0.1:${port}`,
            'Stop Server'
        ).then(selection => {
        ...
        vscode.window.showInformationMessage(
            'SuiteView AI Bridge server is not running',
            'Start Server'
        ).then(selection => {
...
async function handleChat(req: http.IncomingMessage, res: http.ServerResponse) {
    ...
    try {
        ...
        if (model) {
            ...
            if (filtered.length === 0) {
                filtered = selectedModels.filter(m => 
        ...
        const chatMessages: vscode.LanguageModelChatMessage[] = messages.map((m: any) => {
            if (m.role === 'user') {
...
// Read file contents
async function handleReadFile(req: http.IncomingMessage, res: http.ServerResponse) {
    const body = await parseBody(req);
...
// Write file contents
async function handleWriteFile(req: http.IncomingMessage, res: http.ServerResponse) {
    const body = await parseBody(req);
...
async function handleListFiles(req: http.IncomingMessage, res: http.ServerResponse) {
    const body = await parseBody(req);
...
async function handleTerminalExecute(req: http.IncomingMessage, res: http.ServerResponse) {
    ...
    try {
        ...
        exec(command, { cwd: workingDir, maxBuffer: 1024 * 1024 * 10 }, (error: any, stdout: string, stderr: string) => {
...
// Get workspace info
async function handleWorkspaceInfo(req: http.IncomingMessage, res: http.ServerResponse) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
...
function getAgentResponsesFilePath(): string {
...
async function handleAgentChat(req: http.IncomingMessage, res: http.ServerResponse) {
    const body = await parseBody(req);
...
async function handleGetAgentResponses(req: http.IncomingMessage, res: http.ServerResponse) {
...
async function handleOpenAIModels(req: http.IncomingMessage, res: http.ServerResponse) {
    try {
        const allModels = await vscode.lm.selectChatModels();
...
async function handleOpenAIChatCompletions(req: http.IncomingMessage, res: http.ServerResponse) {
    const body = await parseBody(req);
    ...
    try {
        ...
        if (stream) {
            ...
            const promptTokens = messages.reduce((acc: number, m: any) => {
