' Module: clsTreeView.cls
' Type: Standard Module
' Stream Path: VBA/clsTreeView
' =========================================================
...
Private moActiveNode As clsTreeViewNode     'JKP: refers to the selected node
Private moEditNode As clsTreeViewNode       'PT the node in EditMode
Private moMoveNode As clsTreeViewNode       'PT node waiting to be moved
Private moRootHolder As clsTreeViewNode     'PT parent for the root node(s), although a clsTreeViewNode it's not a real node
...
Event Click(cNode As clsTreeViewNode)       'Node click event
' MouseAction generic example for all mouse events. See notes in NodeEventRouter
' Event MouseAction(cNode As clsTreeViewNode, Action As Long, Button As Integer, Shift As Integer, X As Single, Y As Single)
...
Public Property Get MoveCopyNode(Optional bMove As Boolean, Optional lColor As Long) As clsTreeViewNode
    bMove = mbMove
    Set MoveCopyNode = moMoveNode
End Property
Public Property Set MoveCopyNode(Optional bMove As Boolean, Optional lColor As Long, cNode As clsTreeViewNode)
...
' To add & remove nodes use clsTreeViewNode.AddChild() or clsTreeView.NodeAdd and clsTeevView.NodeRemove()
...
Public Function AddRoot(Optional skey As String, Optional vCaption, Optional vImageMain, _
                        Optional vImageExpanded) As clsTreeViewNode

    On Error GoTo errH

    If moRootHolder Is Nothing Then
        Set moRootHolder = New clsTreeViewNode
...
Public Sub Copy(cSource As clsTreeViewNode, cDest As clsTreeViewNode, _
...
Public Sub Move(cSource As clsTreeViewNode, cDest As clsTreeViewNode, _
                Optional vBefore, Optional ByVal vAfter, _
                Optional ByVal bShowError As Boolean)
...
Dim idx As Long
Dim cNode As clsTreeViewNode
Dim cSourceParent As clsTreeViewNode
...
Public Function NodeAdd(Optional vRelative, _
                        ...
                        Optional vImageExpanded) As clsTreeViewNode    '  As tvTreevRelationship
...
'    see also clsTreeViewNode.AddChild

    Dim i As Long
    Dim cNode As clsTreeViewNode
    Dim cRelative As clsTreeViewNode
    Dim cParent As clsTreeViewNode
    Dim cTmp As clsTreeViewNode
...
errH:
    ...
    If Erl = 100 And Err.Number = 457 Then
        Err.Raise vbObjectError + 1, "clsTreeViewNode.AddChild", "Duplicate key: '" & skey & "'"
    Else
        #If DebugMode = 1 Then
            Stop
            Resume
        #End If
        Err.Raise Err.Number, "clsTreeViewNode.AddChild", Err.Description
    End If
End Function

Public Sub NodeRemove(cNode As clsTreeViewNode)
...
'    Call Refresh() when done removing nodes
    ...
    Dim cParent As clsTreeViewNode
    Dim cNodeAbove As clsTreeViewNode, cNd As clsTreeViewNode
...
Public Sub ScrollToView(Optional cNode As clsTreeViewNode, _
...
'*    although visible throughout the project these are only intended to be called by clsTreeViewNodes *
'***********************************************************************************************

Friend Property Get EditMode(cNode As clsTreeViewNode) As Boolean  ' PT
    EditMode = mbEditMode
End Property

Friend Property Let EditMode(cNode As clsTreeViewNode, ByVal bEditMode As Boolean)  ' PT
...
Friend Function RaiseAfterLabelEdit(cNode As clsTreeViewNode, sNewText As String) As Boolean
...
Friend Sub NodeEventRouter(cNode As clsTreeViewNode, sControl As String, lAction As tvMouse, _
...
Dim cLastChild As clsTreeViewNode
    ...
    If sControl = "Caption" Then
        ...
        Else ' tvDblClick, tvDown, tvMove, tvUp
            ...
            '   uncomment moTree.NodeEventRouter in mctlControl_MouseDown, _MouseUp and _DblClick in clsTreeViewNode
...
Private Function AddNodeToCol(colNodes As Collection, cAddNode As clsTreeViewNode, bTreeCol As Boolean, Optional vBefore, Optional vAfter)
    Dim i As Long
    Dim skey As String
    Dim cTmp As clsTreeViewNode
    Dim pos As Long
...
Private Sub BuildTree(cNode As clsTreeViewNode, ByVal lLevel As Long, Optional lMaxLevel As Long = -1)
    Dim cChild As clsTreeViewNode
...
Private Sub BuildNodeControls(cNode As clsTreeViewNode, ByVal lLevel As Long)
...
Private Sub Clone(cParent As clsTreeViewNode, cNode As clsTreeViewNode, Optional vBefore, Optional ByVal vAfter)
' PT clone a node and add the 4-way references
    Dim bTriStateOrig As Boolean
    Dim cClone As clsTreeViewNode
    Dim cChild As clsTreeViewNode
    ...
    Set cClone = New clsTreeViewNode
...
Private Function NextVisibleNodeInTree(ByRef cStartNode As clsTreeViewNode, Optional bUp As Boolean = True) As clsTreeViewNode
'-------------------------------------------------------------------------
...
Private Function NodeIsVisible(Optional cNode As clsTreeViewNode, Optional lngCntVisible As Long) As Boolean
...
Private Sub ResetActiveNodeColor(cNode As clsTreeViewNode)
    Dim lbColor As Long
    Dim lFColor As Long
    If Not cNode Is Nothing Then
...
Private Sub SetTreeExpansionLevel(lLevel As Long, Optional lCurLevel As Long, _
                                          Optional cNode As clsTreeViewNode, Optional bExit As Boolean = False)
'-------------------------------------------------------------------------
