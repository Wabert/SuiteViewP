' Module: mdl_cpearson_array_tools.bas
' Type: Standard Module
' Stream Path: VBA/mdl_cpearson_array_tools
' =========================================================

Attribute VB_Name = "mdl_cpearson_array_tools"
Option Explicit
Option Compare Text
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' modArraySupport
...
'Haessly 7/15/2017.  Changed Res from Integer to Long so it could handle arr with > 65k rows
...
Else
    ...
    ' Redim Result.
    '''''''''''''''
    ReDim result(LBound(arr, 1) To UBound(arr, 1), LBound(arr, 2) To UBound(arr, 2) + AdditionalElements)
    ''''''''''''''''''''''''''''''
    ' Transfer Arr array to Result
    ''''''''''''''''''''''''''''''
    For RowNdx = LBound(arr, 1) To UBound(arr, 1)
...
'=======================================================================================
'Functions below are not part of cpearson_array_tools package above but are copied and
'modified from the functions in this package
'=======================================================================================
...
'''''''''''''''''''''''''''''''''
' Ensure InputArray is a 2
' dimensional array.
'''''''''''''''''''''''''''''''''
If NumberOfArrayDimensions(arr:=InputArray) <> 2 Then
    Exit Function
End If

'''''''''''''''''''''''''''''''''
' Ensure RowArray is a 1
' dimensional array.
'''''''''''''''''''''''''''''''''
If NumberOfArrayDimensions(arr:=RowArray) <> 1 Then
    Exit Function
End If
...
Public Function DeleteRowFromArray(InputArray As Variant, Index As Long) As Boolean
'RJH - 6/25/2016
'Copied
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' DeleteRowFromArray
' This function creates a new array from InputArray and stores the results back into InputArray
' InputArray must be a 2D dynamic array.  All rows upto Index are populated.  Then all rows
' after Index are populated.  Rows at Index are not populated into new array
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Dim Ndx As Long

DeleteRowFromArray = False
...
' Ensure Index is a valid element index.
' We allow Index to be equal to
' UBound or LBound or anywhere inbetween.
'''''''''''''''''''''''''''''''''''''''''
If (Index < LBound(InputArray, 1)) Or (Index > UBound(InputArray, 1)) Then
    Exit Function
End If

'Create new array with one less row than InputArray but the same number of columns
Dim temparray() As Variant
ReDim temparray(LBound(InputArray, 1) To UBound(InputArray, 1) - 1, LBound(InputArray, 2) To UBound(InputArray, 2))
...
'Add all rows after Index
For Ndx = UBound(InputArray, 1) To Index + 1 Step -1
 For ColumnNdx = LBound(InputArray, 2) To UBound(InputArray, 2) Step 1
   temparray(Ndx - 1, ColumnNdx) = InputArray(Ndx, ColumnNdx)
 Next
Next
...
Next
InputArray = temparray
DeleteRowFromArray = True

End Function

Public Function DeleteColumnFromArray(InputArray As Variant, Index As Long) As Boolean
'RJH - 12/13/2016

'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' DeleteColumnFromArray
' This function creates a new array from InputArray and then stores the result back to InputArray.
' InputArray must be a 2D dynamic array.  All columns up to Index are populated.  Then all columns
' after Index are populated.  Entires at Column Index are not populated into new array
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Dim Ndx As Long

DeleteColumnFromArray = False
''''''''''''''''''''''''''''''''
' Ensure InputArray is an array.
''''''''''''''''''''''''''''''''
If IsArray(InputArray) = False Then
...
' UBound or LBound or anywhere inbetween.
'''''''''''''''''''''''''''''''''''''''''
If (Index < LBound(InputArray, 2)) Or (Index > UBound(InputArray, 2)) Then
    Exit Function
End If

'Create new array with one less column than InputArray but the same number of rows
Dim temparray() As Variant
ReDim temparray(LBound(InputArray, 1) To UBound(InputArray, 1), LBound(InputArray, 2) To UBound(InputArray, 2) - 1)

Dim ColumnNdx As Long
Dim RowNdx As Long

'Add all columns after Index
For ColumnNdx = Index + 1 To UBound(InputArray, 2) Step 1
 For RowNdx = LBound(InputArray, 1) To UBound(InputArray, 1) Step 1
   temparray(RowNdx, ColumnNdx - 1) = InputArray(RowNdx, ColumnNdx)
 Next
Next

'Add all Columns prior to Index
For ColumnNdx = LBound(InputArray, 1) To Index - 1 Step 1
 For RowNdx = LBound(InputArray, 1) To UBound(InputArray, 1) Step 1
   temparray(RowNdx, ColumnNdx) = InputArray(RowNdx, ColumnNdx)
 Next
Next

InputArray = temparray
DeleteColumnFromArray = True

End Function


Public Function InsertArrayIntoMatrix(InputMatrix As Variant, Index As Long, InsertArray As Variant, Optional ChangeDimension As Integer = 1) As Variant
'RJH - 6/25/2016


'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'
' This function creates a new array from InputMatrix and inserts InsertArray at locatation Index.
' InputMatrix must be a 2D dynamic array. InsertArray must be a 1D array with the same number of
' elements as columns in InputMatrix. A new matrix is created with one more row than InputMatrix.
' All rows upto Index are populated.  Then InsertArray is added, then the rest of the rows from
' InputMatrix are added. The InputMatrix is then set equal to the new array.  The value of Index
' must be greater than or equal to the LBound of InputMatrix and less than or equal to UBound+1.
' If Index is UBound+1, the Value is placed at the end of the array.
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Dim Ndx As Long
Dim ExpDim As Long  'The dimension that is Expanding
Dim StatDim As Long 'The dimension that will remain the same

'Ensure valid ChangeDimension
If ChangeDimension <> 1 And ChangeDimension <> 2 Then
    Exit Function
End If
...
' Ensure InputArray is an array.
''''''''''''''''''''''''''''''''
If IsArray(InsertArray) = False Then
    Exit Function
End If
...
' Ensure InputArray is dynamic.
''''''''''''''''''''''''''''''''
If IsArrayDynamic(arr:=InputMatrix) = False Then
    Exit Function
End If
...
' Ensure InputArray is allocated.
'''''''''''''''''''''''''''''''''
If IsArrayAllocated(arr:=InputMatrix) = False Then
    Exit Function
End If
...
' dimensional array.
'''''''''''''''''''''''''''''''''
If NumberOfArrayDimensions(arr:=InputMatrix) <> 2 Then
    Exit Function
End If
...
' dimensional array.
'''''''''''''''''''''''''''''''''
If NumberOfArrayDimensions(arr:=InsertArray) <> 1 Then
    Exit Function
End If
...
If ChangeDimension = 1 Then
    
    ' Ensure Index is a valid element index.
    If (Index < LBound(InputMatrix, 1)) Or (Index > UBound(InputMatrix, 1) + 1) Then
        Exit Function
    End If
    
    'Make sure InsertArray has enough elements to fit into InputMatrix
    If UBound(InsertArray) - LBound(InsertArray) <> UBound(InputMatrix, 2) - LBound(InputMatrix, 2) Then
      Exit Function
    End If
    
    'Create new array with one higher on dimension 1
    ReDim temparray(LBound(InputMatrix, 1) To UBound(InputMatrix, 1) + 1, LBound(InputMatrix, 2) To UBound(InputMatrix, 2))

    
    'Add all entries after Index
    For Ndx1 = UBound(InputMatrix, 1) To Index Step -1
     For Ndx2 = LBound(InputMatrix, 2) To UBound(InputMatrix, 2) Step 1
       temparray(Ndx1 + 1, Ndx2) = InputMatrix(Ndx1, Ndx2)
     Next
    Next
    
    'Insert array at new target index
    For Ndx2 = LBound(InputMatrix, 2) To UBound(InputMatrix, 2) Step 1
      temparray(Index, Ndx2) = InsertArray(Ndx2)
    Next
    
    
    'Add all entries after Index
    For Ndx1 = Index - 1 To LBound(InputMatrix, 1) Step -1
     For Ndx2 = LBound(InputMatrix, 2) To UBound(InputMatrix, 2) Step 1
       temparray(Ndx1, Ndx2) = InputMatrix(Ndx1, Ndx2)
     Next
    Next

Else 'ChangeDimension = 2

    ' Ensure Index is a valid element index.
    If (Index < LBound(InputMatrix, 2)) Or (Index > UBound(InputMatrix, 2) + 1) Then
        Exit Function
    End If
    
    'Make sure InsertArray has enough elements to fit into InputMatrix
    If UBound(InsertArray) - LBound(InsertArray) <> UBound(InputMatrix, 1) - LBound(InputMatrix, 1) Then
      Exit Function
    End If
    
    'Create new array with one higher on dimension 1

    ReDim temparray(LBound(InputMatrix, 1) To UBound(InputMatrix, 1), LBound(InputMatrix, 2) To UBound(InputMatrix, 2) + 1)


    'Add all entries after Index
    For Ndx1 = LBound(InputMatrix, 1) To UBound(InputMatrix, 1) Step 1
     For Ndx2 = UBound(InputMatrix, 2) To Index Step -1
       temparray(Ndx1, Ndx2 + 1) = InputMatrix(Ndx1, Ndx2)
     Next
    Next
    
    'Insert array at new target index
    For Ndx1 = LBound(InputMatrix, 1) To UBound(InputMatrix, 1) Step 1
      temparray(Ndx1, Index) = InsertArray(Ndx1)
    Next
    
    
    'Add all entries after Index
    For Ndx1 = LBound(InputMatrix, 1) To UBound(InputMatrix, 1) Step 1
     For Ndx2 = Index - 1 To UBound(InputMatrix, 2) Step -1
       temparray(Ndx1, Ndx2) = InputMatrix(Ndx1, Ndx2)
     Next
    Next
...
InputMatrix = temparray

InsertArrayIntoMatrix = True
