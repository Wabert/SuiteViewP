...
def credit_interest(
    ...
    if config.interest_method == "ExactDays":
        # Daily compound: (1 + annual_rate)^(days/days_in_year) - 1
        monthly_rate = (1.0 + effective_annual_rate) ** (days / days_in_year) - 1.0
    else:
        # Monthly compounding
    ...
    if total_loaned > 0.0 and av_after_deduction > 0.0:
        ...
        if config.interest_method == "ExactDays":
            reg_credit_monthly = (1.0 + reg_credit_annual) ** (days / days_in_year) - 1.0
            pref_credit_monthly = (1.0 + pref_credit_annual) ** (days / days_in_year) - 1.0
        else:
