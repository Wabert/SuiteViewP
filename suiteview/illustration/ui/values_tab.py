...
class IllustrationValuesTab(QWidget):
    ...
    def display_projection(
        ...
        self.grid.set_numeric_formatting(
            default_decimals=2,
            column_decimals={"Face Amount": 0, "Year": 0, "Month": 0, "Attained Age": 0},
        )
    ...
    def _state_to_row(self, policy: IllustrationPolicyData, state: MonthlyState) -> dict:
        return {
            ...
            "Guideline Limit": state.guideline_limit,
            "Forceout": state.guideline_forceout,
            "GP Exception Gross": state.gp_exception_prem_gross,
            "GP Exception Prem": state.gp_exception_prem,
            "Exception Mode": state.exception_prem_mode,
            "Exception Protection": state.exception_protection,
            "Premium Cap": state.premium_cap,
            "Premium Capped": state.premium_capped,
            "7-Pay Accum": state.accumulated_7pay,
            "TAMRA Year": state.tamra_year,
