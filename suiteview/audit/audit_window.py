...
class AuditWindow(FramelessWindowBase):
    ...
    def build_content(self) -> QWidget:
        ...
        # ── Bottom bar ──────────────────────────────────────────────
        # Separator line above the bottom bar
        sep_line = QFrame()
        ...
        bottom = QHBoxLayout()
        bottom.setSpacing(6)
        bottom.setContentsMargins(4, 3, 4, 3)
    ...
    def _build_sql(self) -> str:
        """Build the audit SQL from all wired-up Policy tab controls."""
        ...
        pt = self.policy_tab  # shorthand
        dt = self.display_tab  # shorthand
        p2t = self.policy2_tab  # shorthand
        at = self.adv_tab  # shorthand
        ...
        # ── Coverages tab flags ──────────────────────────────────
        ct = self.coverages_tab
        cov_val_class = ct.val_class.text().strip()
        cov_val_base = ct.val_base.text().strip()
        cov_val_sub = ct.val_sub.text().strip()
        cov_val_mort = ct.val_mort_table.text().strip()
        cov_rpu_mort = ct.rpu_mort_table.text().strip()
        cov_eti_mort = ct.eti_mort_table.text().strip()
        cov_nfo_rate = ct.nfo_int_rate.text().strip()
        cov_val_class_ne = ct.chk_val_class_ne_plan.isChecked()
        cov_multi_base = ct.chk_multiple_base.isChecked()
        cov_gio = ct.chk_cov_gio.isChecked()
        cov_cola = ct.chk_cov_cola.isChecked()
        cov_skipped_rein = ct.chk_skipped_cov_rein.isChecked()
        cov_cv_rate = ct.chk_cv_rate_gt_zero.isChecked()
        cov_gcv_gt_cv = ct.chk_gcv_gt_cv.isChecked()
        cov_gcv_lt_cv = ct.chk_gcv_lt_cv.isChecked()
        cov_non_trad = bool(ct.chk_non_trad.isChecked() and ct.list_non_trad.selectedItems())
        cov_spec_amt_lo = ct.txt_spec_amt_lo.text().strip()
        cov_spec_amt_hi = ct.txt_spec_amt_hi.text().strip()
        cov_has_spec_amt = bool(cov_spec_amt_lo or cov_spec_amt_hi)
        cov_init_term = bool(ct.chk_init_term.isChecked() and ct.list_init_term.selectedItems())

        # Base coverage column
        _bw = ct.base_cov_widgets
        ...
        rider1_info = _rider_info(ct.rider1_widgets)
        rider2_info = _rider_info(ct.rider2_widgets)
        ...
        if has_term_entry:
            ...
            _tw = "WHERE 1=1"
            if _term_lo:
                _tw += f" AND PRE_TERMINATION_DATES.TERM_ENTRY_DT >= '{_esc(_term_lo)}'"
            if _term_hi:
                _tw += f" AND PRE_TERMINATION_DATES.TERM_ENTRY_DT <= '{_esc(_term_hi)}'"
            ...
            if fund_items:
                ...
                for fid in fund_items:
                    parts.append(
                        ...
                        f" WHERE FND_ID_CD = '{_esc(fid)}' AND FND_ALC_PCT > 0 AND FND_ALC_TYP_CD = 'P'")
        ...
        disp_trad_rates = self.display_tab.Checkbox_DisplayTradRates.isChecked()
        if disp_trad_rates:
        ...
        if has_fund_values:
            ...
            if _fid:
                sql_parts.append(f"    AND FUND_VALUES.FND_ID_CD = '{_esc(_fid)}'")
        if adv_prem_alloc:
        ...
        for i in range(3):
            ...
            sql_parts.append(f"    AND {alias}.SPM_BNF_TYP_CD = '{_esc(ben_code)}'")
            if sub_type:
                sql_parts.append(f"    AND {alias}.SPM_BNF_SBY_CD = '{_esc(sub_type)}'")
            if post_issue:
                sql_parts.append(f"    AND {alias}.BNF_ISS_DT > COVERAGE1.ISSUE_DT")
            if cease_lo:
                sql_parts.append(f"    AND {alias}.BNF_CEA_DT >= '{_esc(cease_lo)}'")
            if cease_hi:
                sql_parts.append(f"    AND {alias}.BNF_CEA_DT <= '{_esc(cease_hi)}'")
            if cease_status:
        ...
        def _emit_rider_joins(info: dict, alias: str, idx: int):
            ...
            pc = info["plancode"]
            if pc:
                sql_parts.append(f"    AND {alias}.PLN_DES_SER_CD = '{_esc(pc)}'")
            pl = info["prod_line"]
            if pl:
                code = pl[0]  # first char
                sql_parts.append(f"    AND {alias}.PRD_LIN_TYP_CD = '{_esc(code)}'")
            sex02 = info["sex_code_02"]
            if sex02:
                code = sex02[0]
                sql_parts.append(f"    AND {alias}.INS_SEX_CD = '{_esc(code)}'")
            person = info["person"]
            if person:
                code = person[:2]  # 2-char code
                sql_parts.append(f"    AND {alias}.PRS_CD = '{_esc(code)}'")
            if info["post_issue"]:
                sql_parts.append(f"    AND {alias}.ISSUE_DT > COVERAGE1.ISSUE_DT")
            issue_lo = info["issue_date_lo"]
            if issue_lo:
                sql_parts.append(f"    AND {alias}.ISSUE_DT >= '{_esc(issue_lo)}'")
            issue_hi = info["issue_date_hi"]
            if issue_hi:
                sql_parts.append(f"    AND {alias}.ISSUE_DT <= '{_esc(issue_hi)}'")
            ct_val = info["change_type"]
            if ct_val:
                code = ct_val[0]
                sql_parts.append(f"    AND {alias}.NXT_CHG_TYP_CD = '{_esc(code)}'")
            change_lo = info["change_date_lo"]
            if change_lo:
                sql_parts.append(f"    AND {alias}.NXT_CHG_DT >= '{_esc(change_lo)}'")
            change_hi = info["change_date_hi"]
            if change_hi:
                sql_parts.append(f"    AND {alias}.NXT_CHG_DT <= '{_esc(change_hi)}'")
            lives = info["lives_cov"]
            if lives:
                code = lives[0]
                sql_parts.append(f"    AND {alias}.LIVES_COV_CD = '{_esc(code)}'")
            ...
            if info["needs_covmod"]:
                ...
                if pi:
                    code = pi[0]
                    sql_parts.append(f"    AND {covmod_alias}.AN_PRD_ID = '{_esc(code)}'")
                cola = info["cola_ind"]
                if cola:
                    sql_parts.append(f"    AND {covmod_alias}.COLA_INCR_IND = '{_esc(cola)}'")
                gio = info["gio_fio"]
                if gio:
                    if gio.lower() == "blank":
                        sql_parts.append(f"    AND {covmod_alias}.OPT_EXER_IND = ''")
                    else:
                        sql_parts.append(f"    AND {covmod_alias}.OPT_EXER_IND = '{_esc(gio)}'")
            ...
            if info["needs_renewals"]:
                ...
                if rc:
                    code = rc[0]
                    sql_parts.append(f"    AND {rnl_alias}.RT_CLS_CD = '{_esc(code)}'")
                sx67 = info["sex_code_67"]
                if sx67:
                    code = sx67[0]
                    sql_parts.append(f"    AND {rnl_alias}.RT_SEX_CD = '{_esc(code)}'")
        ...
        if sys_code:
            wheres.append(f"POLICY1.CK_SYS_CD = '{_esc(sys_code)}'")

        # -- Policy tab: Plancode (searches all coverages) --
        plancode = pt.txt_plancode.text().strip()
        if plancode:
            wheres.append(f"COVSALL.PLN_DES_SER_CD = '{_esc(plancode)}'")

        # -- Plancode tab: multiple plancodes (IN list) --
        plancode_list = self.plancode_tab.get_plancodes()
        if plancode_list:
            wheres.append(
                f"COVSALL.PLN_DES_SER_CD IN ({_in_list(plancode_list)})")
        ...
        if company:
            ...
            wheres.append(f"POLICY1.CK_CMP_CD = '{_esc(co_code)}'")
        ...
        if form_num:
            wheres.append(f"COVERAGE1.POL_FRM_NBR LIKE '{_esc(form_num)}%'")

        # -- Policy tab: Branch --
        branch = pt.txt_branch.text().strip()
        if branch:
            wheres.append(f"SUBSTR(POLICY1.SVC_AGC_NBR, 2, 3) = '{_esc(branch)}'")
        ...
        if polnum:
            ...
            if criteria == "Starts with":
                wheres.append(f"POLICY1.CK_POLICY_NBR LIKE '{_esc(polnum)}%'")
        ...
        if pt.chk_status_code.isChecked():
            codes = _selected_codes(pt.list_status)
            if codes:
                wheres.append(f"POLICY1.PRM_PAY_STA_REA_CD IN ({_in_list(codes)})")

        # -- Policy tab: Product line code (checkbox + listbox → all covs) --
        if pt.chk_product_line.isChecked():
            codes = _selected_codes(pt.list_product_line)
            if codes:
                wheres.append(f"COVSALL.PRD_LIN_TYP_CD IN ({_in_list(codes)})")

        # -- Policy tab: Product indicator (checkbox + listbox → any cov) --
        if pt.chk_product_indicator.isChecked():
            codes = _selected_codes(pt.list_product_indicator)
            if codes:
                wheres.append(f"COVSALL.ADV_PRD_IND IN ({_in_list(codes)})")
        ...
        if pt.chk_state.isChecked():
            ...
            if abbrevs:
                ...
                if st_codes:
                    wheres.append(f"POLICY1.POL_ISS_ST_CD IN ({_in_list(st_codes)})")

        # -- Policy tab: Last entry code (checkbox + listbox) --
        if pt.chk_last_entry.isChecked():
            codes = _selected_codes(pt.list_last_entry)
            if codes:
                wheres.append(f"POLICY1.LST_ETR_CD IN ({_in_list(codes)})")

        # -- Policy tab: Suspense code (checkbox + listbox) --
        if pt.chk_suspense.isChecked():
            codes = _selected_codes(pt.list_suspense)
            if codes:
                wheres.append(f"POLICY1.SUS_CD IN ({_in_list(codes)})")

        # -- Policy tab: Billing form (checkbox + listbox) --
        if pt.chk_billing_form.isChecked():
            codes = _selected_codes(pt.list_billing_form)
            if codes:
                wheres.append(f"POLICY1.BIL_FRM_CD IN ({_in_list(codes)})")
        ...
        _add_int_range(wheres, "COVERAGE1.INS_ISS_AGE",
        ...
        _add_date_range(wheres, "POLICY1.LST_FIN_DT",
        ...
        if p2t.chk_loan_type.isChecked():
            codes = _selected_codes(p2t.list_loan_type)
            if codes:
                wheres.append(f"POLICY1.LN_TYP_CD IN ({_in_list(codes)})")
        # -- Loan charge Rate (01) --
        ...
        _add_decimal_range(wheres, "POLICYDEBT.LOAN_PRINCIPLE",
                           p2t.txt_total_loan_prin_lo, p2t.txt_total_loan_prin_hi)
        _add_decimal_range(wheres, "POLICYDEBT.LOAN_ACCRUED",
                           p2t.txt_total_accured_lint_lo, p2t.txt_total_accured_lint_hi)
        # -- Trad Overloan Ind (01) --
        if p2t.chk_trad_overloan.isChecked():
            codes = _selected_codes(p2t.list_trad_overloan)
            if codes:
                wheres.append(f"POLICY1_MOD.OVERLOAN_IND IN ({_in_list(codes)})")
        # -- Non Trad Indicator (02) --
        if p2t.chk_non_trad.isChecked():
            codes = _selected_codes(p2t.list_non_trad)
            if codes:
                wheres.append(f"POLICY1.NON_TRD_POL_IND IN ({_in_list(codes)})")
        # -- Scheduled Loan Payment (20) --
        if p2t.chk_std_loan_payment.isChecked():
            codes = _selected_codes(p2t.list_std_loan_payment)
            if codes:
                wheres.append(f"SLR_BILL_CONTROL.BIL_FRM_CD IN ({_in_list(codes)})")
        # -- Definition of Life Insurance (66) --
        if p2t.chk_def_life.isChecked():
            codes = _selected_codes(p2t.list_def_life)
            if codes:
                wheres.append(f"NONTRAD.TFDF_CD IN ({_in_list(codes)})")
        # -- Reinsurance Code --
        if p2t.chk_reinsurance.isChecked():
            codes = _selected_codes(p2t.list_reinsurance)
            if codes:
                wheres.append(f"POLICY1.REINSURED_CD IN ({_in_list(codes)})")
        # -- Has Change Seq (68) --
        if has_change_seq:
            codes = _selected_codes(p2t.list_change_seq)
            if codes:
                wheres.append(f"CHANGE_SEGMENT.CHG_TYP_CD IN ({_in_list(codes)})")
        ...
        if adv_grace_rule:
            codes = _selected_codes(at.list_grace_rule)
            if codes:
                wheres.append(f"NONTRAD.GRA_THD_RLE_CD IN ({_in_list(codes)})")
        # -- Death Benefit Option (66) --
        if adv_db_option:
            codes = _selected_codes(at.list_db_option)
            if codes:
                wheres.append(f"NONTRAD.DTH_BNF_PLN_OPT_CD IN ({_in_list(codes)})")
        # -- Orig Entry Code (01) --
        if adv_orig_entry:
            codes = _selected_codes(at.list_orig_entry)
            if codes:
                wheres.append(f"POLICY1.OGN_ETR_CD IN ({_in_list(codes)})")
        ...
        _add_decimal_range(wheres, "MVVAL.CSV_AMT",
                           at.rng_accum_val[0], at.rng_accum_val[1])
        # -- Shadow Account Value (58) --
        _add_decimal_range(wheres, "SHADOWAV.TAR_PRM_AMT",
                           at.rng_shadow_acct[0], at.rng_shadow_acct[1])
        # -- Current Specified Amount (02) --
        _add_decimal_range(wheres, "COVSUMMARY.TOTAL_SA",
                           at.rng_curr_spec_amt[0], at.rng_curr_spec_amt[1])
        # -- Accum MTP (58) --
        _add_decimal_range(wheres, "ACCUMMTP.TAR_PRM_AMT",
                           at.rng_accum_mtp[0], at.rng_accum_mtp[1])
        # -- Accum GLP (58) --
        _add_decimal_range(wheres, "ACCUMGLP.TAR_PRM_AMT",
                           at.rng_accum_glp[0], at.rng_accum_glp[1])
        # -- Type P Sequence (57) --
        _add_int_range(wheres, "ALLOCATION_P_COUNT.FND_ALC_SEQ_NBR",
                       at.rng_type_p[0], at.rng_type_p[1])
        # -- Type V Sequence (57) --
        _add_int_range(wheres, "ALLOCATION_V_COUNT.FND_ALC_SEQ_NBR",
        ...
        if cov_val_class:
            wheres.append(f"COVERAGE1.INS_CLS_CD = '{_esc(cov_val_class)}'")
        if cov_val_base:
            wheres.append(f"COVERAGE1.PLN_BSE_SRE_CD = '{_esc(cov_val_base)}'")
        if cov_val_sub:
            wheres.append(f"COVERAGE1.LIF_PLN_SUB_SRE_CD = '{_esc(cov_val_sub)}'")
        if cov_val_mort:
            wheres.append(f"COVERAGE1.MTL_FCT_TBL_CD = '{_esc(cov_val_mort)}'")
        if cov_rpu_mort:
            wheres.append(f"COVERAGE1.NSP_RPU_TBL_CD = '{_esc(cov_rpu_mort)}'")
        if cov_eti_mort:
            wheres.append(f"COVERAGE1.NSP_EI_TBL_CD = '{_esc(cov_eti_mort)}'")
        if cov_nfo_rate:
            try:
        ...
        # Non-trad indicator
        if cov_non_trad:
            codes = _selected_codes(ct.list_non_trad)
            if codes:
                wheres.append(f"POLICY1.NON_TRD_POL_IND IN ({_in_list(codes)})")
        # Current Specified Amount range
        _add_decimal_range(wheres, "COVSUMMARY.TOTAL_SA",
                           ct.txt_spec_amt_lo, ct.txt_spec_amt_hi)
        # Initial Term Period
        if cov_init_term:
            codes = _selected_codes(ct.list_init_term)
            if codes:
                wheres.append(f"COVERAGE1.INT_RNL_PER IN ({_in_list(codes)})")

        # -- Base coverage column (Col 4) --
        if cov_base_plancode:
            wheres.append(f"COVERAGE1.PLN_DES_SER_CD = '{_esc(cov_base_plancode)}'")
        if cov_base_prod_line:
            code = cov_base_prod_line[0]
            wheres.append(f"COVERAGE1.PRD_LIN_TYP_CD = '{_esc(code)}'")
        if cov_base_form_number:
            wheres.append(f"COVERAGE1.POL_FRM_NBR LIKE '{_esc(cov_base_form_number)}%'")
        if cov_base_sex02:
            code = cov_base_sex02[0]
            wheres.append(f"COVERAGE1.INS_SEX_CD = '{_esc(code)}'")
        if cov_base_person:
            code = cov_base_person[:2]
            wheres.append(f"COVERAGE1.PRS_CD = '{_esc(code)}'")
        if cov_base_lives_cov:
            code = cov_base_lives_cov[0]
            wheres.append(f"COVERAGE1.LIVES_COV_CD = '{_esc(code)}'")
        if cov_base_change_type:
            code = cov_base_change_type[0]
            wheres.append(f"COVERAGE1.NXT_CHG_TYP_CD = '{_esc(code)}'")
        _add_date_range(wheres, "COVERAGE1.ISSUE_DT",
                        _bw["issue_date_lo"], _bw["issue_date_hi"])
        _add_date_range(wheres, "COVERAGE1.NXT_CHG_DT",
        ...
        if cov_base_prod_ind:
            ...
            wheres.append(f"MODCOV1.AN_PRD_ID = '{_esc(code)}'")
        if cov_base_cola_ind:
            wheres.append(f"MODCOV1.COLA_INCR_IND = '{_esc(cov_base_cola_ind)}'")
        if cov_base_gio_fio:
            if cov_base_gio_fio.lower() == "blank":
                wheres.append("MODCOV1.OPT_EXER_IND = ''")
            else:
                wheres.append(f"MODCOV1.OPT_EXER_IND = '{_esc(cov_base_gio_fio)}'")
        # Base coverage RENEWALS fields (rateclass, sex67)
        if cov_base_rateclass:
            code = cov_base_rateclass[0]
            wheres.append(f"COV1_RENEWALS.RT_CLS_CD = '{_esc(code)}'")
        if cov_base_sex67:
            code = cov_base_sex67[0]
            wheres.append(f"COV1_RENEWALS.RT_SEX_CD = '{_esc(code)}'")
    ...
    def _run_audit(self):
        ...
        try:
            ...
            self.lbl_query_time.setText(f"Query time:  {_fmt_time(t_query)}")
            self.lbl_print_time.setText(f"Print time:  {_fmt_time(t_print)}")
            self.lbl_total_time.setText(f"Total time:  {_fmt_time(t_total)}")
    ...
    def _open_polview_with_policy(self, policy_number: str,
        ...
        # Create once; reuse across close/reopen to keep policy history
...
def _esc(val: str) -> str:
    """Escape single quotes for DB2 SQL."""
    return val.replace("'", "''")


def _selected_codes(listbox) -> list[str]:
...
def _in_list(codes: list[str]) -> str:
    ...
    return ", ".join(f"'{_esc(c)}'" for c in codes)


def _add_int_range(wheres: list, column: str, lo_widget, hi_widget):
...
def _add_date_range(wheres: list, column: str, lo_widget, hi_widget):
    """Append date >= / <= clauses if the range widgets have values."""
    lo = lo_widget.text().strip()
    hi = hi_widget.text().strip()
    if lo:
        wheres.append(f"{column} >= '{_esc(lo)}'")
    if hi:
        wheres.append(f"{column} <= '{_esc(hi)}'")


def _add_decimal_range(wheres: list, column: str, lo_widget, hi_widget):
