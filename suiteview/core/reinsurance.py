...
TAI_CESSION_COLUMNS = [
    "_Co", "_Pol", "_Cov", "_CessSeq", "_ReinsCo", "_RepCo",
    "_FromDt", "_ToDt", "_RepDt", "_PolDt", "_ReinsDt", "_PolStatus",
    "_Treaty", "_TreatyRef", "_TreatyGrp", "_TrtyJoinMeth", "_ReinsTyp",
    "_Plan", "_SrchPlan", "_ProdCD", "_Face", "_Retn", "_Ceded",
    "_NAR", "_Filler6", "_monthEnd",
]

# Display-friendly header names (strip leading underscore)
TAI_CESSION_HEADERS = [c.lstrip("_") for c in TAI_CESSION_COLUMNS]
...
def fetch_tai_cession(policy_number: str) -> TAICessionResult:
    ...
    try:
        ...
        cursor.execute(
            "SELECT MAX(_monthEnd) FROM TAICession WHERE _Pol = ?",
            (policy_number,),
        )
        row = cursor.fetchone()
        ...
        if policy_month_end:
            ...
            cursor.execute("SELECT MAX(_monthEnd) FROM TAICession")
