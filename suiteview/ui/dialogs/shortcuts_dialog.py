...
class BookmarkButton(QPushButton):
    ...
    def open_bookmark(self):
        ...
        try:
            ...
            if getattr(e, 'winerror', None) == 1223 or path.lower().endswith('.lnk'):
                QMessageBox.warning(
...
class BookmarksDialog(QDialog):
    ...
    # ============================================================
    # Helper methods for new data format (categories are items)
    # ============================================================
    
    def _get_category_names(self) -> list:
        """Get list of category names from items (new format)"""
        return [item.get('name') for item in self.bookmarks_data.get('items', [])
                if item.get('type') == 'category']
    
    def _find_category_item(self, name: str):
        """Find category item by name (new format)"""
        for item in self.bookmarks_data.get('items', []):
            if item.get('type') == 'category' and item.get('name') == name:
                return item
        return None
    
    def _category_name_exists(self, name: str) -> bool:
        """Check if category name exists (new format)"""
        return self._find_category_item(name) is not None
    ...
    def refresh_categories(self):
        ...
        # Add category panels (new format - categories are items with nested items)
        for item in self.bookmarks_data.get('items', []):
            if item.get('type') == 'category':
                category_name = item.get('name', '')
                bookmarks = item.get('items', [])
    ...
    def populate_category_menu(self, menu):
        ...
        # List existing categories for removal/rename (new format)
        categories = self._get_category_names()
        if categories:
    ...
    def rename_category(self, old_name):
        ...
        if ok and new_name:
            ...
            if self._category_name_exists(new_name):
                ...
                # Find and rename the category (new format)
                category = self._find_category_item(old_name)
                if category:
    ...
    def add_category(self):
        ...
        if ok and category_name:
            ...
            if self._category_name_exists(category_name):
                ...
                # New format - create category as item with nested items
                new_category = self._bookmark_manager.create_category(category_name, [])
                if 'items' not in self.bookmarks_data:
                    self.bookmarks_data['items'] = []
                self.bookmarks_data['items'].append(new_category)
                self.save_bookmarks()
    ...
    def add_link(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            # Add to selected category (new format)
            category_item = self._find_category_item(category)
            if category_item:
                if 'items' not in category_item:
                    category_item['items'] = []
                new_bookmark = self._bookmark_manager.create_bookmark(
                    bookmark_data['name'], bookmark_data['path']
                )
                category_item['items'].append(new_bookmark)
                self.save_bookmarks()
                self.refresh_categories()
    ...
    def add_bookmark_to_category(self, name, path, category=None):
        ...
        # If category specified and doesn't exist, create it (new format)
        if category and not self._category_name_exists(category):
            new_cat = self._bookmark_manager.create_category(category, [])
            self.bookmarks_data.setdefault('items', []).append(new_cat)
        ...
        # Add bookmark to category (new format)
        category_item = self._find_category_item(category)
        if category_item:
            new_bookmark = self._bookmark_manager.create_bookmark(name, path)
            category_item.setdefault('items', []).append(new_bookmark)
            self.save_bookmarks()
            self.refresh_categories()
    ...
    def remove_bookmark(self, category, bookmark_data):
        ...
        try:
            # New format - find category and remove bookmark by path
            category_item = self._find_category_item(category)
            if category_item:
                items = category_item.get('items', [])
                for i, item in enumerate(items):
                    if item.get('path') == bookmark_data.get('path'):
                        items.pop(i)
                        break
            self.save_bookmarks()
            self.refresh_categories()
    ...
    def move_bookmark_to_category(self, bookmark_data, target_category, insert_index=-1):
        ...
        try:
            ...
            # Find and remove from source category (new format)
            source_index = -1
            source_cat_item = self._find_category_item(source_category)
            if source_cat_item:
                for i, item in enumerate(source_cat_item.get('items', [])):
                    if item.get('path') == path:
                        source_cat_item['items'].pop(i)
                        source_index = i
                        break
            ...
            # Get or create target category (new format)
            target_cat_item = self._find_category_item(target_category)
            if not target_cat_item:
                target_cat_item = self._bookmark_manager.create_category(target_category, [])
                self.bookmarks_data.setdefault('items', []).append(target_cat_item)
            
            # Create new bookmark
            new_bookmark = self._bookmark_manager.create_bookmark(
                bookmark_data.get('name', Path(path).name),
                path
            )
            
            # Insert at specific index or append at end
            target_items = target_cat_item.setdefault('items', [])
            if insert_index >= 0 and insert_index <= len(target_items):
                target_items.insert(insert_index, new_bookmark)
            else:
                target_items.append(new_bookmark)
    ...
    def edit_bookmark(self, category, bookmark_data):
        ...
        if edit_dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if new_category != category:
                # Remove from old category (new format)
                old_cat_item = self._find_category_item(category)
                if old_cat_item:
                    for i, item in enumerate(old_cat_item.get('items', [])):
                        if item.get('path') == bookmark_data.get('path'):
                            old_cat_item['items'].pop(i)
                            break
                
                # Re-detect type
                if new_path.startswith('http://') or new_path.startswith('https://'):
                    if 'sharepoint' in new_path.lower():
                        bm_type = detect_sharepoint_type(new_path)
                    else:
                        bm_type = 'url'
                ...
                # Add to new category (new format)
                new_cat_item = self._find_category_item(new_category)
                if new_cat_item:
                    new_bookmark = self._bookmark_manager.create_bookmark(new_name, new_path)
                    new_cat_item.setdefault('items', []).append(new_bookmark)
            else:
                # Update bookmark data in place (new format - find by old path)
                cat_item = self._find_category_item(category)
                if cat_item:
                    for item in cat_item.get('items', []):
                        if item.get('path') == bookmark_data.get('path'):
                            item['name'] = new_name
                            item['path'] = new_path
                            break
    ...
    def remove_category_confirm(self, category_name):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Remove category from items list (new format)
            items = self.bookmarks_data.get('items', [])
            for i, item in enumerate(items):
...
class BookmarkBar(QFrame):
    ...
    def init_ui(self):
        ...
        # Accept drops and forward to internal BookmarkContainer
        self.setAcceptDrops(True)
    ...
    def dragEnterEvent(self, event):
        """Forward drag enter events to the internal BookmarkContainer"""
    ...
    def dragMoveEvent(self, event):
        """Forward drag move events to the internal BookmarkContainer"""
        if hasattr(self, 'bookmark_container'):
    ...
    def dragLeaveEvent(self, event):
        """Forward drag leave events to the internal BookmarkContainer"""
        if hasattr(self, 'bookmark_container'):
            self.bookmark_container.dragLeaveEvent(event)
        super().dragLeaveEvent(event)
    
    def dropEvent(self, event):
        """Forward drop events to the internal BookmarkContainer"""
        if hasattr(self, 'bookmark_container'):
    ...
    def _on_bookmark_dropped(self, bookmark_data):
        ...
        # Remove from category if source_category is a category name (new format)
        if not removed_from_source and source_category and source_category not in ('__QUICK_LINKS__', '__CONTAINER__', '__BAR__', ''):
            # Check bookmark bar categories (new format - categories are items)
            cat_item = self._bookmark_manager.find_category_by_name(source_category)
            if cat_item:
                for i, item in enumerate(cat_item.get('items', [])):
                    if item.get('path') == path:
                        cat_item['items'].pop(i)
    ...
    def _on_file_dropped(self, file_data):
        ...
        path_obj = Path(path)
        from suiteview.ui.widgets.bookmark_data_manager import get_bookmark_manager
        bar_item = get_bookmark_manager().create_bookmark(
            path_obj.name,
            path
        )
                ...
                with open(self.bookmarks_file, 'r') as f:
                    ...
                    if 'bar_items' not in data:
                        ...
                        for bookmark in data.get('bar_bookmarks', []):
                            bar_items.append(manager.create_bookmark(
                                bookmark.get('name', ''),
                                bookmark.get('path', '')
                            ))
                        # Add categories
            ...
            for url in urls:
                ...
                if file_path:
                    ...
                    if category_target and category_target in self.bookmarks_data['categories']:
                        ...
                        from suiteview.ui.widgets.bookmark_data_manager import get_bookmark_manager
                        self.bookmarks_data['bar_items'].append(get_bookmark_manager().create_bookmark(
                            bookmark.get('name', ''),
                            bookmark.get('path', '')
                        ))
