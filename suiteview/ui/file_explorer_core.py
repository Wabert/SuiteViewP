...
class FileExplorerCore(QWidget):
    ...
    def create_tree_panel(self):
        ...
        # Header widget with label and history button (PolView style)
        ...
        # History button (will be connected by FileExplorerTab)
        ...
        """)
        header_layout.addWidget(self.folders_history_btn)
    ...
    def create_file_item(self, path, dir_entry=None):
        ...
        is_network = str(path).startswith('\\\\')
        stat_result = None if is_network else self._stat_path(path, dir_entry)
    ...
    def create_details_panel(self):
        ...
        # Home button for depth search - go back to top level folder (hidden by default)
        ...
        self.depth_home_btn.setStyleSheet("""
            QPushButton {
        ...
        self.depth_home_btn.hide()  # Hidden by default, shown when depth search is active
        header_layout.addWidget(self.depth_home_btn)
                ...
                with os.scandir(str(dir_path)) as entries:
                    for entry in entries:
                        try:
                            ...
                            item_path = Path(entry.path)
                            # Keep the DirEntry for cached stat data (long-path support)
                            items_with_type.append((item_path, is_directory, entry))
                        except (PermissionError, OSError):
                            continue
                ...
                for item_path, is_directory, dir_entry in items_with_type:
                    try:
                        if is_directory:
                            ...
                            row_items = self.create_file_item(item_path, dir_entry=dir_entry)
    ...
    def navigate_to_bookmark_folder(self, folder_path):
        ...
        print(f"[DEBUG] navigate_to_bookmark_folder called with: {folder_path}")
        try:
            # Check if it's a URL
            if folder_path and (folder_path.lower().startswith('http://') or folder_path.lower().startswith('https://')):
                # Open URL in default browser
                print(f"[DEBUG] Opening URL in browser: {folder_path}")
                import webbrowser
