...
class AssessmentPanel(QWidget):
    ...
    def _show_partial_premium_breakdown(self):
        ...
        (min face) premium.  Same layout as the Calc Premium breakdown
        on the Policy Info screen."""
        bd = self._partial_prem_breakdown
        if not bd:
            return

        dlg = QDialog(self)
        dlg.setWindowTitle("Partial Premium Calculation Breakdown")
        ...
        # Title
        title = QLabel(
            f"Premium Breakdown - Min Face ${bd['face_amount']:,.0f}"
        ...
        # ── Separate PW riders from non-PW riders ─────────────
        pw_riders = [r for r in bd.get('rider_details', [])
                     if isinstance(r, dict) and r.get('rider_type') == 'PW']
        other_riders = [r for r in bd.get('rider_details', [])
                        if isinstance(r, dict) and r.get('rider_type') != 'PW']

        # ── Coverage 1: Base plan + PW ────────────────────────
        add_header("Coverage 1")
        add_line("Plan", bd['plan_code'], indent=True)
        rate_sex = bd.get('rate_sex', bd['sex'])
        add_line("Issue age/Sex/Class",
                 f"{bd['issue_age']} / {rate_sex} / {bd['rate_class']}", indent=True)
        add_line("Rate", f"{bd['base_rate']:.4f}" if bd['base_rate'] else "0", indent=True)
        add_line("Table Rating", str(bd['table_rating']), indent=True)
        add_line("Rating factor", f"{bd['substandard_factor']:.3f}", indent=True)
        flat_val = bd.get('flat_rate_applied', 0)
        add_line("Flat Extra", f"{flat_val:.2f}" if flat_val > 0 else "0", indent=True)
        add_line("Units", f"{bd['face_units']:.3f}", indent=True)

        # PW rider info (shown within Coverage 1)
        for pw in pw_riders:
            add_spacer()
            pw_rate = pw.get('rate')
            add_line("PW Rate",
                     f"{pw_rate:.4f}" if pw_rate is not None else "—", indent=True)
            pw_sub = pw.get('pw_sub', 1.0)
            add_line("PW Rating Factor",
                     f"{pw_sub:.0f}" if pw_sub == int(pw_sub) else f"{pw_sub:.2f}",
                     indent=True)

        # Coverage 1 premium = step3 + PW premiums
        cov1_premium = bd['step3_premium']
        for pw in pw_riders:
            cov1_premium += pw['premium']
        add_spacer()
        add_premium_line("Premium", f"{cov1_premium:,.4f}", indent=True)

        # ── Coverage N: Other riders ──────────────────────────
        cov_num = 2
        for rdr in other_riders:
            add_spacer()
            add_header(f"Coverage {cov_num}")
            add_line("Plan", rdr['plancode'], indent=True)
            r_sex = rdr.get('sex', '')
            r_class = rdr.get('rate_class', '0')
            r_age = rdr.get('issue_age', '')
            add_line("Issue age/Sex/Class",
                     f"{r_age} / {r_sex} / {r_class}", indent=True)
            r_rate = rdr.get('rate')
            add_line("Rate",
                     f"{r_rate:.4f}" if r_rate is not None else "—", indent=True)
            r_tbl = rdr.get('table_rating', 0)
            r_factor = 1.0 + r_tbl * 0.25
            add_line("Table Rating", f"{r_factor:.3f}", indent=True)
            add_line("table factor", str(r_tbl), indent=True)
            add_line("Flat Extra", "0", indent=True)
            add_line("Units", f"{rdr['face_units']:.3f}", indent=True)
            add_spacer()
            add_premium_line("Premium", f"{rdr['premium']:,.4f}", indent=True)
            cov_num += 1

        # ── Policy Fee / Modal / Calculated ────────────────────
        add_spacer()
        add_divider()
        add_line("Policy Fee", f"{bd['policy_fee']:,.3f}")
        add_line("Modal Factor", f"{bd['modal_factor']:.4f}")
        add_premium_line("Calculated Modal Prem", f"{bd['calc_modal']:,.2f}")
