...
Opened from the "orion_pcr3_r" button on the Policy Support tab's left nav panel.
...
class OrionPcrTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        controls.addWidget(self._mk_label("monthEnd"))
        ...
        self.query_btn = QPushButton("orion_pcr3_r")
    ...
    def load_policy(self, policy: Optional['PolicyInformation']):
        """Bind a policy and reset the default monthEnd range."""
        self._policy = policy
        self._has_access = _ul_rates_available()
        self._apply_access_state()
    ...
    def _set_default_dates(self):
        """Default the start to three months ago; leave the end blank (no upper limit)."""
    ...
    def _on_query(self):
        ...
        # Match on policy number only (no company code).  monthEnd is a YYYYMM
        # string, so the bounds are inclusive of the start and end months.
    ...
    def _run_query(self, sql: str, params: tuple) -> pd.DataFrame:
        ...
        Returns every column with monthEnd moved to the front.
        """
        import pyodbc
