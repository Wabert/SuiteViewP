...
class PolicyTab(QWidget):
    ...
    # ── direct DB loading (legacy fallback) ──────────────────────────────

    # TODO: Dead code – never called; main_window uses load_data_from_policy() exclusively.
    def load_data(self, db: DB2Connection, where_clause: str, policy_info: dict = None):
        try:
            self._db_data = {}
            ...
            self._db_data["LH_BAS_POL"] = bas_pol

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.TH_COV_PHA WHERE {where_clause} AND COV_PHA_NBR = 1"
                )
                cov_pha = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                cov_pha = {}
            self._db_data["TH_COV_PHA"] = cov_pha

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.LH_COV_PHA WHERE {where_clause} AND COV_PHA_NBR = 1"
                )
                lh_cov_pha = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                lh_cov_pha = {}
            self._db_data["LH_COV_PHA"] = lh_cov_pha

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.LH_BIL_FRM_CTL WHERE {where_clause}"
                )
                bil_ctl = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                bil_ctl = {}
            self._db_data["LH_BIL_FRM_CTL"] = bil_ctl

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.TH_USER_REPLACEMENT WHERE {where_clause} AND SEGMENT_TYPE = 'R'"
                )
                repl = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                repl = {}
            self._db_data["TH_USER_REPLACEMENT"] = repl

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.TH_USER_GENERIC WHERE {where_clause}"
                )
                gen = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                gen = {}
            self._db_data["TH_USER_GENERIC"] = gen

            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT * FROM DB2TAB.LH_FXD_PRM_POL WHERE {where_clause}"
                )
                fxd = dict(zip(cols, rows[0])) if rows else {}
            except Exception:
                fxd = {}
            self._db_data["LH_FXD_PRM_POL"] = fxd

            self._populate_column1(bas_pol, cov_pha, policy_info)
            self._populate_column2(bas_pol, cov_pha, bil_ctl, repl, gen, lh_cov_pha)
            self._populate_column3(bas_pol, policy_info, fxd, lh_cov_pha)

        except Exception as e:
            pass

    # ── populate helpers (direct DB path) ────────────────────────────────

    def _populate_column1(self, bas_pol: dict, cov_pha: dict, policy_info: dict = None):
        c = self.col1
        if policy_info:
            c.set_value("pol_number", policy_info.get("PolicyNumber", bas_pol.get("POL_NBR", "")))
        else:
            c.set_value("pol_number", bas_pol.get("POL_NBR", ""))
        c.set_value("company", translate_company_code(str(bas_pol.get("CO_CD", ""))))
        c.set_value("plancode", str(cov_pha.get("PLN_DES_SER_CD", bas_pol.get("PLN_CD", ""))).strip())
        prod_line = str(bas_pol.get("PRD_LN_CD", ""))
        c.set_value("prod_line", f"{prod_line} - {translate_product_line_code(prod_line)}")
        c.set_value("an_prd_id", cov_pha.get("AN_PRD_ID", ""))
        c.set_value("non_trd_ind", str(bas_pol.get("NON_TRD_POL_IND", "")))

        state_cd = bas_pol.get("POL_ISS_ST_CD")
        try:
            state_abbr = translate_state_code(int(state_cd)) if state_cd else ""
            c.set_value("issue_state", state_abbr)
        except (ValueError, TypeError):
            c.set_value("issue_state", str(state_cd) if state_cd else "")
        c.set_value("prm_pay_sta", bas_pol.get("PRM_PAY_STA_REA_CD", ""))
        c.set_value("sus_cd", bas_pol.get("SUS_CD", ""))
        grace_val = str(bas_pol.get("GRC_IND", "0"))
        c.set_value("in_grace", f"{grace_val} - {translate_grace_indicator(grace_val)}")
        c.set_value("gpe_date", format_date(bas_pol.get("GRC_PRD_END_DT", US_DATE_FMT)))

        c.set_value("prm_paid_to", format_date(bas_pol.get("PRM_PAID_TO_DT", US_DATE_FMT)))
        c.set_value("prm_bill_to", format_date(bas_pol.get("PRM_BILL_TO_DT", US_DATE_FMT)))
        c.set_value("app_wrt_dt", format_date(bas_pol.get("APP_WRT_DT", US_DATE_FMT)))
        c.set_value("lst_anv_dt", format_date(bas_pol.get("LST_ANV_DT", US_DATE_FMT)))
        c.set_value("nxt_bil_dt", format_date(bas_pol.get("NXT_BIL_DT", US_DATE_FMT)))
        c.set_value("nxt_sch_not", format_date(bas_pol.get("NXT_SCH_NOT_DT", US_DATE_FMT)))
        c.set_value("nxt_sch_stt", format_date(bas_pol.get("NXT_SCH_STT_DT", US_DATE_FMT)))
        c.set_value("nxt_mvry_prc", format_date(bas_pol.get("NXT_MVRY_PRC_DT", US_DATE_FMT)))
        c.set_value("nxt_yr_end", format_date(bas_pol.get("NXT_YR_END_PRC_DT", US_DATE_FMT)))
        c.set_value("lst_fin_dt2", format_date(bas_pol.get("LST_FIN_DT", US_DATE_FMT)))

        c.set_value("pol_1035", bas_pol.get("POL_1035_XCG_IND", ""))
        c.set_value("idt_prm_ind", bas_pol.get("IDT_PRM_IND", ""))
        tfdf = str(bas_pol.get("TFDF_GDL_IND", ""))
        c.set_value("tfdf_gdl", f"{tfdf} - {translate_tefra_defra_ind(tfdf)}" if tfdf else "")
        c.set_value("int_mlv_nbr", bas_pol.get("INT_MLV_NBR", ""))
        rein_cd = str(bas_pol.get("REINSURED_CD", "")).strip()
        c.set_value("reinsured", translate_reinsurance_code(rein_cd))

    def _populate_column2(self, bas_pol: dict, cov_pha: dict, bil_ctl: dict,
                          repl: dict, gen: dict, lh_cov_pha: dict = None):
        if lh_cov_pha is None:
            lh_cov_pha = self._db_data.get("LH_COV_PHA", {})
        c = self.col2

        prm_mode = translate_bill_mode_from_frequency(
            bas_pol.get("PMT_FQY_PER", ""),
            bas_pol.get("NSD_MD_CD", "")
        )
        c.set_value("prm_mode", prm_mode)
        c.set_value("modal_prm", format_currency(bas_pol.get("POL_PRM_AMT"), "$"))

        bil_form = str(bas_pol.get("BIL_FRM_CD", ""))
        c.set_value("bil_form", translate_bill_form_code(bil_form))
        c.set_value("bil_ctl_nbr", bil_ctl.get("BIL_CTL_NBR", "not found"))

        if repl.get("SEGMENT_TYPE") == "R":
            c.set_value("replaced_pol", str(repl.get("REPLACED_POLICY", "")).strip())
        else:
            c.set_value("replaced_pol", "")

        ogn = str(bas_pol.get("OGN_ETR_CD", ""))
        c.set_value("ogn_etr_cd", f"{ogn} - {translate_entry_code(ogn)}")
        conv_pol_val = str(gen.get("EXCH_POL_NUMBER", "")).strip()
        c.set_value("conv_pol", conv_pol_val if conv_pol_val else "Null")
        lst = str(bas_pol.get("LST_ETR_CD", ""))
        c.set_value("lst_etr_cd", f"{lst} - {translate_last_entry_code(lst)}")

        usr_res = str(bas_pol.get("USR_RES_CD", ""))
        c.set_value("mdo", usr_res[:1] if usr_res else "")
        c.set_value("bypass_lapse", usr_res[-1:] if len(usr_res) > 1 else "")
        mec = str(bas_pol.get("MEC_STATUS_CD", ""))
        c.set_value("mec_status", translate_mec_indicator(mec))

        prod_line = str(bas_pol.get("PRD_LN_CD", ""))
        nfo = str(bas_pol.get("NFO_OPT_TYP_CD", ""))
        if prod_line == "U":
            c.set_value("nfo_opt", "Surrender value")
        else:
            c.set_value("nfo_opt", f"{nfo} - {translate_nfo_code(nfo)}")

        pri_div = str(bas_pol.get("PRI_DIV_OPT_CD", ""))
        c.set_value("pri_div_opt", f"{pri_div} - {translate_div_option_code(pri_div)}")
        div_2nd = str(bas_pol.get("DIV_2ND_OPT_CD", ""))
        c.set_value("div_2nd_opt", f"{div_2nd} - {translate_div_option_code(div_2nd)}")

        mtl_cd = str(lh_cov_pha.get("MTL_FCT_TBL_CD", "")).strip()
        c.set_value("mtl_tbl", mtl_cd)
        c.set_value("mtl_desc", translate_mortality_table_code(mtl_cd))
        res_rt = lh_cov_pha.get("RES_ITS_RT")
        c.set_value("res_its_rt", format_rate(res_rt, decimals=2, suffix="%"))
        c.set_value("mtl_fun_cd", str(lh_cov_pha.get("MTL_FUN_CD", "")))

        nsp_ei_cd = str(lh_cov_pha.get("NSP_EI_TBL_CD", "")).strip()
        c.set_value("nsp_ei_tbl", nsp_ei_cd)
        c.set_value("nsp_ei_desc", translate_mortality_table_code(nsp_ei_cd))
        nsp_rpu_cd = str(lh_cov_pha.get("NSP_RPU_TBL_CD", "")).strip()
        c.set_value("nsp_rpu_tbl", nsp_rpu_cd)
        c.set_value("nsp_rpu_desc", translate_mortality_table_code(nsp_rpu_cd))
        nsp_rt = lh_cov_pha.get("NSP_ITS_RT")
        if nsp_rt and str(nsp_rt) != "Null":
            c.set_value("nsp_its_rt", format_rate(nsp_rt, decimals=2, suffix="%"))
        else:
            c.set_value("nsp_its_rt", "")

    def _populate_column3(self, bas_pol: dict, policy_info: dict, fxd: dict, lh_cov_pha: dict = None):
        if lh_cov_pha is None:
            lh_cov_pha = self._db_data.get("LH_COV_PHA", {})
        c = self.col3

        svc_agc = str(bas_pol.get("SVC_AGC_NBR", "")).strip()
        company_code = str(bas_pol.get("CO_CD", ""))
        ...
        usr_res3 = str(bas_pol.get("USR_RES_CD", ""))
        c.set_value("mdo3", usr_res3[:1] if usr_res3 else "")
        c.set_value("svc_agent", str(bas_pol.get("SVC_AGT_NBR", "")).strip())

        c.set_value("class_cd", lh_cov_pha.get("INS_CLS_CD", ""))
        c.set_value("base_cd", lh_cov_pha.get("PLN_BSE_SRE_CD", ""))
        c.set_value("sub_cd", lh_cov_pha.get("LIF_PLN_SUB_SRE_CD", ""))

        ln_typ = str(bas_pol.get("LN_TYP_CD", ""))
        if ln_typ == "9":
            ...
            if ln_typ not in ("6", "7"):
                c.set_value("ln_rate", format_rate(bas_pol.get("LN_PLN_ITS_RT"), decimals=2, suffix="%"))
            else:
                c.set_value("ln_rate", "")

        prod_line = str(bas_pol.get("PRD_LN_CD", ""))
        if prod_line == "0":
            c.set_value("san_md_fct", fxd.get("SAN_MD_FCT", ""))
            c.set_value("qtr_md_fct", fxd.get("QTR_MD_FCT", ""))
            c.set_value("mo_md_fct", fxd.get("MO_MD_FCT", ""))
            md_ord = str(fxd.get("MD_PRM_MUL_ORD_CD", "")).strip()
            c.set_value("md_prm_ord", f"{md_ord} - {translate_multiply_order_code(md_ord)}" if md_ord else "")
            rt_ord = str(fxd.get("RT_FCT_ORD_CD", "")).strip()
            c.set_value("rt_fct_ord", f"{rt_ord} - {translate_rating_order_code(rt_ord)}" if rt_ord else "")
            c.set_value("rou_rle_cd", fxd.get("ROU_RLE_CD", ""))
        else:
            for f in ["san_md_fct", "qtr_md_fct", "mo_md_fct",
                      "md_prm_ord", "rt_fct_ord", "rou_rle_cd"]:
                c.set_value(f, "")

    # ── helpers ──────────────────────────────────────────────────────────

    def _is_in_grace(self, bas_pol: dict) -> bool:
        sus_cd = str(bas_pol.get("SUS_CD", "")).strip()
        return sus_cd in ("G", "2", "3")

    def _get_mortality_desc(self, table_code) -> str:
        if not table_code:
            return ""
        return translate_mortality_table_code(str(table_code).strip())
