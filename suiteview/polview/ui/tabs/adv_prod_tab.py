...
class AdvProdValuesTab(QWidget):
    ...
    def load_data(self, db: DB2Connection, where_clause: str):
        try:
            self._load_policy_info(db, where_clause)
            self._load_monthliversary_values(db, where_clause)
            self._load_fund_history(db, where_clause)
            self._load_fund_summary(db, where_clause)
        except Exception as e:
            pass

    def _load_policy_info(self, db, where_clause):
        try:
            cols, rows = db.execute_query_with_headers(
                f"SELECT * FROM DB2TAB.LH_POL_MVRY_VAL WHERE {where_clause} ORDER BY MVRY_DT DESC FETCH FIRST 1 ROW ONLY"
            )
            if rows:
                mv_data = dict(zip(cols, rows[0]))
                self.policy_info.set_value("total_av", format_currency(mv_data.get("CSV_AMT", 0) or 0))

            cols, rows = db.execute_query_with_headers(
                f"SELECT SUM(FND_VAL_AMT) AS TOTAL_FUND FROM DB2TAB.LH_POL_FND_VAL_TOT WHERE {where_clause}"
            )
            if rows:
                self.policy_info.set_value("unimpaired_av", format_currency(dict(zip(cols, rows[0])).get("TOTAL_FUND", 0) or 0))

            impaired = 0
            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT SUM(LN_PRC_AMT) AS TOTAL_LOAN FROM DB2TAB.LH_POL_LN WHERE {where_clause}"
                )
                if rows:
                    impaired = dict(zip(cols, rows[0])).get("TOTAL_LOAN", 0) or 0
            except Exception:
                pass
            self.policy_info.set_value("impaired_av", format_currency(impaired))
            ...
            try:
                ...
                if rows:
                    ntp = dict(zip(cols, rows[0]))
                    if ntp.get("GAV_AMT"):
                        self.policy_info.set_value("gav", format_currency(ntp["GAV_AMT"]))
                    if ntp.get("CCV_AMT"):
                        self.policy_info.set_value("ccv", format_currency(ntp["CCV_AMT"]))
                    guar_rate = ntp.get("GUR_ITS_RT")
                    if guar_rate:
                        try:
                            self.policy_info.set_value("guar_int_rate", f"{float(guar_rate)/100:.2%}")
                        except Exception:
                            self.policy_info.set_value("guar_int_rate", str(guar_rate))
                    self.policy_info.set_value("grace_rule_code", str(ntp.get("GRA_THD_RLE_CD", "")))
            except Exception:
                pass
            ...
            try:
                ...
                if rows:
                    ug = dict(zip(cols, rows[0]))
                    spd = ug.get("INITIAL_PAY_DUR")
                    if spd and int(spd) > 0:
                        self.policy_info.set_value("short_pay_dur", str(spd))
                        self.policy_info.set_value("short_pay_mode", str(ug.get("INITIAL_MODE", "")))
                        dta = ug.get("DIAL_TO_PREM_AGE")
                        if dta and int(dta) > 0:
                            self.policy_info.set_value("db_dial_to_age", str(dta))
            except Exception:
                pass
        except Exception as e:
            pass

    def _load_monthliversary_values(self, db, where_clause):
        try:
            cols, rows = db.execute_query_with_headers(
                f"SELECT * FROM DB2TAB.LH_POL_MVRY_VAL WHERE {where_clause} ORDER BY MVRY_DT DESC"
            )
            table_rows = []
            for row in rows:
                d = dict(zip(cols, row))
                table_rows.append([
                    format_date(d.get("MVRY_DT")),
                    str(d.get("POL_DUR_NBR", "")),
                    str(d.get("POL_MTH_NBR", "")),
                    format_currency(d.get("GUA_RT_ERN_ITS_AMT")),
                    format_currency(d.get("CSV_AMT")),
                    format_currency(d.get("CINS_AMT")),
                    format_currency(d.get("OTH_PRM_AMT")),
                    format_currency(d.get("EXP_CRG_AMT")),
                    format_currency(d.get("NAR_AMT")),
                ])
            self.mv_values.load_table_data(table_rows)
        except Exception as e:
            pass

    def _load_fund_history(self, db, where_clause):
        try:
            cols, rows = db.execute_query_with_headers(
                f"SELECT * FROM DB2TAB.LH_POL_FND_VAL_TOT WHERE {where_clause}"
            )
            table_rows = []
            for row in rows:
                d = dict(zip(cols, row))
                table_rows.append([
                    str(d.get("FND_CD", "")),
                    str(d.get("PHA_CD", "")),
                    format_date(d.get("MVRY_DT", d.get("EFF_DT"))),
                    format_date(d.get("STR_DT", d.get("EFF_DT"))),
                    format_currency(d.get("FND_VAL_AMT")),
                ])
            self.fund_history.load_table_data(table_rows)
        except Exception as e:
            pass

    def _load_fund_summary(self, db, where_clause):
        try:
            cols, rows = db.execute_query_with_headers(
                f"SELECT FND_CD, SUM(FND_VAL_AMT) AS AMOUNT FROM DB2TAB.LH_POL_FND_VAL_TOT WHERE {where_clause} GROUP BY FND_CD"
            )
            self.unimpaired_values.load_table_data([
                [str(dict(zip(cols, r)).get("FND_CD", "")), format_currency(dict(zip(cols, r)).get("AMOUNT"))]
                for r in rows
            ])
            impaired_rows = []
            try:
                cols, rows = db.execute_query_with_headers(
                    f"SELECT FND_CD, SUM(LN_PRC_AMT) AS AMOUNT FROM DB2TAB.LH_POL_LN WHERE {where_clause} GROUP BY FND_CD"
                )
                impaired_rows = [
                    [str(dict(zip(cols, r)).get("FND_CD", "")), format_currency(dict(zip(cols, r)).get("AMOUNT"))]
                    for r in rows
                ...
                pass
            self.impaired_values.load_table_data(impaired_rows)
        except Exception as e:
            pass
