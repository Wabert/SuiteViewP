# Script to replace FilterPopup class with optimized version

# Read the original file
with open(r'c:\Dev\SuiteViewP\suiteview\ui\widgets\filter_table_view.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Find the start and end of FilterPopup class
...
if start_pos == -1 or end_pos == -1:
    print("ERROR: Could not find FilterPopup class boundaries")
    exit(1)

# Extract the parts
before_filterpopup = content[:start_pos + 1]  # Include the newline
after_filterpopup = content[end_pos + 1:]  # Include the newline before FilterTableView

# The new optimized FilterPopup class
new_filterpopup = '''
...
# Also need to add QListView and QAbstractListModel to imports
import_line = "from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QTableView,"
new_import_line = "from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QTableView, QListView,"

before_filterpopup = before_filterpopup.replace(import_line, new_import_line)

core_import = "from PyQt6.QtCore import Qt, QAbstractTableModel, QModelIndex, QSortFilterProxyModel, pyqtSignal, QRect, QPoint"
new_core_import = "from PyQt6.QtCore import Qt, QAbstractTableModel, QModelIndex, QSortFilterProxyModel, pyqtSignal, QRect, QPoint, QAbstractListModel, QTimer"

before_filterpopup = before_filterpopup.replace(core_import, new_core_import)

# Assemble the new content
new_content = before_filterpopup + new_filterpopup + after_filterpopup

# Write the new file
with open(r'c:\Dev\SuiteViewP\suiteview\ui\widgets\filter_table_view.py', 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Successfully replaced FilterPopup class")
print(f"Before FilterPopup: {len(before_filterpopup)} chars")
print(f"New FilterPopup: {len(new_filterpopup)} chars")  
print(f"After FilterPopup: {len(after_filterpopup)} chars")
print(f"Total new file: {len(new_content)} chars")
